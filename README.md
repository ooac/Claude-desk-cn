...
- 兼容 Kimi 网关：保留 Claude Code 默认模型 `opus[1m]` 时，可将其映射到 Kimi 网关返回的 `kimi-for-coding`。
...
7. 安装原生菜单语言资源。
8. 替换部分硬编码英文文案。
9. 为 Kimi 网关补充 `opus[1m] -> kimi-for-coding` 默认模型映射。
...
如果使用 Kimi 网关，同时 Claude Code 设置里保留 `opus[1m]`，本项目会在前端模型识别层把它映射到 `kimi-for-coding`。这不会修改你的 `~/.claude/settings.json`。
