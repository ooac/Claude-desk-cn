...
- 兼容 Kimi 网关：保留 Claude Code 默认模型 `opus[1m]`，并在前端模型识别层兼容只返回 `kimi-for-coding` 的模型列表。
...
7. 安装原生菜单语言资源。
8. 替换部分硬编码英文文案。
9. 为 Kimi 网关补充 `opus[1m]` 默认模型识别兼容，不把默认值改写成 `kimi-for-coding`。
...
如果使用 Kimi 网关，同时 Claude Code 设置里保留 `opus[1m]`，本项目会在前端模型识别层把 `opus[1m]` 视为有效默认模型，即使网关模型列表只返回 `kimi-for-coding`。界面和本地设置仍保留 `opus[1m]`，不会写成 `kimi-for-coding`。
